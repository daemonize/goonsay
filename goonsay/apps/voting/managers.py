...
class VoteManager(models.Manager):
    ...
    def record_vote(self, obj, ip_address, vote):
        """
        Record an IP Address's vote on a given object. Only allows a given IP
        to vote X amount of times, though that vote may be changed.
        ...
        ctype = ContentType.objects.get_for_model(obj)
        try:
            v = self.get(ip_address=ip_address, content_type=ctype,
            ...
            if vote != 0:
                self.create(ip_address=ip_address, content_type=ctype,
    ...
    def get_for_ip_address(self, obj, ip_address):
        """
        Get the vote made on the given object by the given address, or
    ...
    def get_for_ip_address_in_bulk(self, objects, ip_address):
        """
        Get a dictionary mapping object ids to votes made by the given
        address on the corresponding objects.
        """
        vote_dict = {}
        if len(objects) > 0:
            ctype = ContentType.objects.get_for_model(objects[0])
