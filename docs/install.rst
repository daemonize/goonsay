...
If you have the project source checked out already, place it in a ``goonsay``
directory inside the virtual environment source (``src``) directory. 
Otherwise, checkout the source now::

    pip install -e git+ssh://git@github.com/daemonize/goonsay.git#egg=goonsay

Next, install the requirements and the project source::

    pip install -r src/goonsay/requirements.pip
...
management file to ``bin`` so it can be run from anywhere::
    ...
    ln -s src/goonsay/goonsay/bin/manage.py ./bin
